/* ==========================================================================
   ...
   Synced live across every device via Firebase Realtime Database, gated by
   a 4-digit family passcode (see README for setup).
   ========================================================================== */
...
(function () {
  ...
  function focusDigit(i) {
    if (digitEls[i]) digitEls[i].focus();
  }
  ...
  function showGateError(msg) {
    ...
    void gateInputsWrap.offsetWidth; // restart animation
  ...
  function subscribeToChores() {
    ...
    unsubscribeChores = onValue(choresRef, function (snapshot) {
      ...
      if (!val) state.chores = [];
      else if (Array.isArray(val)) state.chores = val.filter(Boolean);
      else state.chores = Object.values(val);
      state.loading = false;
      state.dbError = false;
      render();
    }, function (err) {
  ...
  function saveChores() {
    set(choresRef, state.chores).catch(function (err) {
  ...
  function render() {
    ...
    if (state.loading) {
      appEl.innerHTML = '<div class="loading-state">Loading your chores… ✨</div>';
      return;
    }
    ...
    if (state.dbError) {
      html += '<div class="storage-banner">📡 Couldn\'t reach the shared family database just now — changes may not be saving. Check your connection and try again.</div>';
    }
